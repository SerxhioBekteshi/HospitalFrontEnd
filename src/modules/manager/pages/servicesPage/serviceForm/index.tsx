import React, { useCallback, useEffect, useRef, useState } from "react";
import { Controller, useForm, useFormContext } from "react-hook-form";
import { Button, Col, Form, FormFeedback, Input, Label, Row } from "reactstrap";
import eFormMode from "../../../../../main/assets/enums/eFormMode";
import SearchSelect from "../../../../../main/components/ServerSideSelect";
import formValidationManager from "../../../../../main/utils/formValidationManager";

const ServiceForm = (props: any) => {
  const { model, mode } = props;
  const { control } = useFormContext<any>();

  return (
    <Form onSubmit={(e) => e.preventDefault()}>
      <Row>
        <Col md={12}>
          <Controller
            control={control}
            name="name"
            render={({ field: { onChange, value }, fieldState: { error } }) => (
              <>
                <Label htmlFor="Name">Name</Label>
                <Input
                  id="Name"
                  type="text"
                  value={value}
                  onChange={onChange}
                  placeholder="Name"
                  invalid={error !== undefined}
                />
                <FormFeedback>
                  {formValidationManager.extractError(error)}
                </FormFeedback>
              </>
            )}
          />
        </Col>

        {mode == eFormMode.Edit && (
          <Col md={12} style={{ marginTop: "1rem" }}>
            <Controller
              control={control}
              name="isActive"
              render={({
                field: { onChange, value },
                fieldState: { error },
              }) => (
                <>
                  <Label htmlFor="isActive" style={{ marginRight: " 1rem" }}>
                    Active
                  </Label>
                  <Input
                    id="isActive"
                    type="checkbox"
                    name="isActive"
                    value={value}
                    checked={value}
                    onChange={(e) => {
                      onChange(e.target.checked);
                    }}
                    invalid={error !== undefined}
                  />
                  <FormFeedback>
                    {formValidationManager.extractError(error)}
                  </FormFeedback>
                </>
              )}
            />
          </Col>
        )}

        <Col md={12} style={{ marginTop: "1rem" }}>
          <Controller
            control={control}
            name="deviceIds"
            render={({ field: { onChange, value }, fieldState: { error } }) => (
              <>
                <Label htmlFor="deviceIds" style={{ marginRight: " 1rem" }}>
                  Pajisjset
                </Label>
                <SearchSelect
                  controller="/list/service-devices"
                  id="Add Devices"
                  onChange={onChange}
                  value={value}
                />
                <FormFeedback>
                  {formValidationManager.extractError(error)}
                </FormFeedback>
              </>
            )}
          />
        </Col>

        <Col md={12} style={{ marginTop: "1rem" }}>
          <Controller
            control={control}
            name="staffIds"
            render={({ field: { onChange, value }, fieldState: { error } }) => (
              <>
                <Label htmlFor="staffIds" style={{ marginRight: " 1rem" }}>
                  Staff
                </Label>
                <SearchSelect
                  controller="/list/service-staff"
                  id="Add Staff"
                  onChange={onChange}
                  value={value}
                />
                <FormFeedback>
                  {formValidationManager.extractError(error)}
                </FormFeedback>
              </>
            )}
          />
        </Col>
      </Row>
    </Form>
  );
};

export default ServiceForm;
